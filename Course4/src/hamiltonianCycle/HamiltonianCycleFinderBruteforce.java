...
public class HamiltonianCycleFinderBruteforce implements
		IHamiltonianCycleFinder {
	...
	private boolean isValid(boolean[][] graph, int[] cycle) {
		...
		for (int i = 0; i < cycle.length; i++) {
			if (i + 1 < cycle.length){
				if (had[cycle[i]] || !graph[cycle[i]][cycle[i + 1]])
					return false;
			} else if (had[cycle[i]] || !graph[cycle[i]][cycle[0]])
				return false;
			had[cycle[i]] = true;
		}
